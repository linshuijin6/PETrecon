...
from result_eval.evaluate import evaluate_metrics
...
def train(model_pre, radon, train_loader, criterion, optimizer, rank, epoch, log_writer):
    ...
    for iteration, (inputs, Y, sino_label, picLD) in enumerate(train_loader):
        ...
        if iteration % 100 == 0:
            ...
            titles = ["x1", "x2", "x1_denoised", "aver_x", "input_LD", "mid_recon", "sino_p1", "sino_p2", "sino_recon_p1", "sino_recon_p2",
...
def test(model_pre, radon, test_loader, criterion, rank, log):
    ...
    with torch.no_grad():
        ...
        psnr_l, ssim_l = evaluate_metrics(mid_recon_list, label_list)
