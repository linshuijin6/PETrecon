...
def train(model_pre, model_recon, radon, train_loader, criterion, optimizer, rank):
    ...
    for iteration, (inputs, Y, sino_label) in enumerate(train_loader):
        ...
        if iteration % 100 == 0:
            ...
            plt.savefig(f'/mnt/data/linshuijin/PETrecon/log_file/pic_visual/{iteration}_{loss_cur:.4f}.png', format='png', dpi=300, bbox_inches='tight')  # 300 dpi保证高分辨率
...
if __name__ == '__main__':
    ...
    recon_size = 128
    # 配置logging模块
    log_file_path = './log_file/training_log_1104.txt'
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 绕过 GPU 0，只使用 GPU 1 和 GPU 2
