...
mmr = {
        ...
        'zCrystalDimCm':         0.40625, # 晶体纵向尺寸（单位：厘米）
        'transaxialFovCm':       60.0,  # 横断面视野（单位：厘米）
        'span':                  11,    # span 参数，用于定义哪些 LORs 被分组在一个 sinogram bin 中
        'nSegments':             11,    # 段的数量
        'maxRingDiff':           60,    # 最大环差
        'nTofBins':              1,     # TOF bin 的数量
        'coinciWindowWidthNsec': 5.85938, # 符合窗口宽度（单位：纳秒）
        'tofResolutionNsec':     5.85938, # TOF 分辨率（单位：纳秒）
        'tofOffsetNsec':         0      # TOF 偏移（单位：纳秒）
      }
